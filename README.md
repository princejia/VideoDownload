...
- **画质按短边计**：竖屏视频（Reels / 抖音 / TikTok）的 1080x1920 计为 1080p，不会因高度为 1920 而被误降到更低画质
...
- Node.js ≥ 22.5（使用内置 `node:sqlite`）。仓库根的 `.node-version` 已声明版本，`fnm` / `nvm` 等工具会自动识别
- 可选：`yt-dlp`（建议）与 `ffmpeg`（用于合并音视频流）

> 全局 Node 低于 22.5 时，可用版本管理器仅为本项目切换，不影响其他项目：
>
> ```bash
...
复制 `.env.example` 为 `.env`（放在**仓库根目录**，服务启动时自动加载），或直接设置环境变量：
...
| `YTDLP_PATH` | 空 | yt-dlp 可执行文件路径（留空则从 PATH 查找） |
| `SIMULATE` | `auto` | 模拟模式：`auto`/`on`/`off` |

> `.env` 不会覆盖已存在的真实环境变量，因此 Railway / Docker 等平台注入的配置优先生效。该文件已在 `.gitignore` 中，不会被提交。
>
> `DATA_DIR` 等相对路径按进程工作目录解析（`npm run dev` 时为 `server/`）；如需固定到仓库根，请填绝对路径。
...
> **作用范围**：上述 Cookie 会以 `--cookies` 传给 yt-dlp，对**所有站点**生效（文件名中的 `youtube` 只是历史命名）。因此 Instagram、X 等平台在遇到登录墙时也可复用同一份 `cookies.txt`。反过来说，导入的 Cookie 会随每次请求发往对应站点，请只使用专门用于下载的账号。
