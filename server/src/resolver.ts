...
function mapJsonToInfo(
  ...
  for (const f of rawFormats) {
    ...
    const rawHeight = Number(f.height ?? 0) || 0;
    const rawWidth = Number(f.width ?? 0) || 0;
    // 画质按短边计：竖屏视频（Reels/抖音/TikTok）的 1080x1920 是 1080p 而非 1920p，
    // 否则 pickFormat 会因 1920 > 目标值而回落到更低画质。
    const height = rawWidth && rawHeight ? Math.min(rawWidth, rawHeight) : rawHeight;
