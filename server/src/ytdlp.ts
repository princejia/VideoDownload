...
export function buildFormatSelector(quality: string, ext: string, formatId = ''): string {
  ...
  // 画质上限不在这里过滤，改由 buildFormatSortArgs 的 -S res 表达，见该函数注释。
  return `bestvideo[ext=${ext}]+bestaudio/bestvideo+bestaudio/best[ext=${ext}]/best`;
}

/**
 * 用 `-S res:N` 表达画质上限。res 取宽高中的较小值，
 * 而 `[height<=N]` 会把竖屏视频（Reels/抖音/TikTok）的 1080x1920 当成 1920 排除掉，
 * 导致实际下到 540x960。
 */
export function buildFormatSortArgs(quality: string, ext: string): string[] {
  if (quality === 'best' || quality === 'audio' || ext === 'm4a') return [];
  const res = parseInt(quality.replace(/\D/g, ''), 10);
  return Number.isFinite(res) && res > 0 ? ['-S', `res:${res}`] : [];
}
